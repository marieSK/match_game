...
MatchGame.renderCards = function(cardValues, $game) {
  ...
  var replaceValue = Math.floor(Math.random() * 8) + 1;
  ...
  if (flippedCards.length === 2) {
    if (flippedCards[0].data('value') === flippedCards[1].data('value')) {
      ...
      if (countFlipped === 16) {
        ...
        win.src = "./resources/images/win.jpg";
        document.getElementById('game').appendChild(win);
      } else if (flippedCards[0].data('value') === 73) {
        var bazinga = document.createElement('img');
        bazinga.src = "./resources/images/bazinga.jpg";
        document.getElementById('game').appendChild(bazinga);
      }
    } else {
